...
$file = $_SESSION['file_upload'];
$stmt = $connect->prepare("SELECT COUNT(*) FROM Content WHERE numeFisier = :numeFisier");
$stmt->bindParam(':numeFisier', $numeFisier, SQLITE3_TEXT);
$result = $stmt->execute();
$numRows = $result->fetchArray(SQLITE3_NUM)[0];


if ($numRows > 0) {
    addValidationError('numeFisier', 'Acestă denumire este deja folosită');
    redirect("../sablon/$file");
}else if($numeFisier == null){
    addValidationError('numeFisier', 'Nu a fost setată denumirea');
    redirect("../sablon/$file");
} else {

    $stmt = $connect->prepare("INSERT INTO `Content` (Content, numeFisier) VALUES (:Content, :numeFisier)");
...
";
            $filename = '../sablon/' . $numeFisier . '.php';
            redirect($filename);
            if (file_put_contents($filename, $fisier)) {
                echo 'Fișierul a fost creat cu succes!';
            } else {
                echo 'Eroare la crearea fișierului!';
            }
        } else {
            echo 'Denumirea șablonului nu a fost furnizată!';
        }
    } else {
        echo 'Acces interzis!';
    }
